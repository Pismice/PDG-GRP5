...
1. Sur cette page d'accueil on peut voir les programmes que l'on a actuellement en cours, on peut voir en vert les séances hebdomadaires déjà effectuées et en gris celles qu'il nous reste à faire d'ici la fin de la semaine.
...
- L'utilisateur peut créer des workouts ou il indique les séances qu'il voudrait faire chaque semaine (il peut mettre plusieurs fois la même séance)
...
- L'utilisateur peut voir ou il est en est dans ses workouts semaine après semaine, c'est à dire qu'il voit les séances qu'il a déjà effectuées et celles qui doivent encore l'être dans la semaine (il ne peut avoir que 1 seul workout actif à la fois)
