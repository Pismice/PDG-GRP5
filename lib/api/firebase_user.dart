  ...
  for (var doc in docs) {
    ...
    for (int i = 0; i < sess.length; ++i) {
      if (sess[i]['start'] != null && sess[i]['end'] != null) {
        totalSecondsSpent += convertTimestampToDateTime(sess[i]['end'])
            .difference(convertTimestampToDateTime(sess[i]['start']))
            .inSeconds;
      }
    }
  }

  if (totalSecondsSpent >= 3600) {
    return "${(totalSecondsSpent / 3600).floor()}h ${(totalSecondsSpent / 60).floor()}m ${totalSecondsSpent % 60}s";
  }
  // si il a passé plus d'une heure
  if (totalSecondsSpent >= 60) {
    // retourne l'affichange avec heure et minute
    return "${(totalSecondsSpent / 60).floor()}m ${totalSecondsSpent % 60}s";
  }
  // sinon on retourne l'affichage avec les minutes
  return "${totalSecondsSpent}s";
}
  ...
  for (var doc in docs) {
    ...
    for (int i = 0; i < sess.length; ++i) {
      for (int j = 0; j < sess[i]['exercises'].length; ++j) {
        Exercise exo = await getExercise(sess[i]['exercises'][j]['id'].id);
        if (exo.type != "WEIGHT") {
          continue;
        }
